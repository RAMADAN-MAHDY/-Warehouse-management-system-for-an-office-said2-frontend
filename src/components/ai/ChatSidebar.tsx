...
export default function ChatSidebar({
  ...
  return (
    <aside className="flex h-full flex-col rounded-[28px] border border-slate-800 bg-slate-950/95 shadow-[0_25px_80px_-40px_rgba(0,0,0,0.9)]">
      <div className="sticky top-0 z-10 flex items-center justify-between gap-2 border-b border-slate-800 bg-slate-950/95 px-3 py-3 sm:px-4">
        <button
          type="button"
          onClick={onCreateNew}
          className="flex flex-1 items-center justify-center gap-2 rounded-2xl border border-emerald-600/30 bg-emerald-600/10 px-4 py-2.5 text-sm font-semibold text-emerald-300 transition hover:border-emerald-500 hover:bg-emerald-600/20 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/40"
        >
        ...
        {onClose && (
          <button
            type="button"
            onClick={onClose}
            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-slate-800 bg-slate-900 text-slate-400 transition hover:bg-slate-800 hover:text-slate-200 lg:hidden"
